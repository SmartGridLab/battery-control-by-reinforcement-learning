...
# 新機能: 分位数回帰の可視化
def quantile_regression_visualization(x, y, taus):
    ...
    def QuantileGradientDescent(X, y, init_theta, tau, lr=1e-4, num_iters=10000):
        ...
        for i in range(num_iters):
            y_hat = X @ theta  # 予測
            delta = y - y_hat  # 誤差
            indic = np.array(delta <= 0., dtype=np.float32)  # インジケータ
            grad = np.abs(tau - indic) * np.sign(delta)  # 勾配
            theta += lr * X.T @ grad  # 更新
        return theta

    # 入力 x に基づいて X（デザイン行列）が適切に定義されるようにします
    X = np.column_stack((np.ones(len(x)), x))

    # 以下を追加して、結果を表示
    plt.figure(figsize=(5,4))
    plt.title("Quantile Regression")
    plt.scatter(x, y, color="gray", s=5)  # サンプル

    for i, tau in enumerate(taus):
        init_theta = np.zeros(2)  # 初期変数
        theta = QuantileGradientDescent(X, y, init_theta, tau)
        y_hat = X @ theta
        plt.plot([min(x), max(x)], [min(y_hat), max(y_hat)], linewidth=2,
                 color=cmap[i], label=str(int(tau * 100))+"%tile")  # 回帰線
...
# 予測が得られるメインのループ内部で：
if i == (p.N_VERIFICATION - 1):
    pv_predict_["upper"] = y_pred[:, 0]  # 予測された上限値で更新
    pv_predict_["lower"] = y_pred[:, 1]  # 予測された下限値で更新

    # quantile_regression_visualization 関数を呼び出す
    quantile_regression_visualization(df_w["hour"], pv_predict_["PVout"].values, np.array([0.1, 0.5, 0.9]))
