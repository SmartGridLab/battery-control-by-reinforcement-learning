...
def battery_operation(self):
    for j in range(len(self.df_result)):
        ...
        # PVが計画よりも少ない場合
            ...
            # 充電量抑制(放電量増加)・売電量変化なし
            self.df_result.at[j, 'charge/discharge_actual_bid'] = self.df_result.at[j, 'charge/discharge_bid'] + abs(delta_PV)    #充電量は負の値なので、値を正の方向へ
            self.df_result.at[j, 'energytransfer_actual_bid'] = self.df_result.at[j, 'energytransfer_bid']

        
            # SoCの計算
            if j == 0:
                previous_soc = 0 ###マジックナンバー
            else:
                previous_soc = self.df_result.at[j, 'SoC_actual_bid']
            soc = previous_soc - (self.df_result.at[j, 'charge/discharge_actual_bid']*0.5)*100/self.BATTERY_CAPACITY
        ...
        if self.df_result.at[j, 'energytransfer_actual_bid'] < 0:
            ...
            self.df_result.at[j, 'mode'] *= 2

        # SoCの最終処理
        if j == 0:
            self.df_result.at[j, 'SoC_actual_bid'] = previous_soc
        if j < last_csv_row:
            self.df_result.at[j+1, 'SoC_actual_bid'] = soc  #最終行以降にsoc格納するスペースはない
