...
for i in range(0,1):
    ...
    if delta_PV >= 0:
        # 充電量増加(放電量抑制)・売電量変化なし
        dataframe.at[j, 'charge/discharge_actual'] = dataframe.at[j, 'charge/discharge_plan'] - abs(delta_PV) #充電量は負の値なので、値を負の方向へ
        dataframe.at[j, 'energytransfer_actual'] = dataframe.at[j, 'energytransfer_plan']
    ...
    # PVが計画よりも少ない場合
        ...
        # 充電量抑制(放電量増加)・売電量変化なし
        dataframe.at[j, 'charge/discharge_actual'] = dataframe.at[j, 'charge/discharge_plan'] + abs(delta_PV)    #充電量は負の値なので、値を正の方向へ
        dataframe.at[j, 'energytransfer_actual'] = dataframe.at[j, 'energytransfer_plan']
